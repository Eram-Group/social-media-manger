...
const SUPPORTED: TPlatformId[] = ['facebook', 'instagram', 'linkedin', 'snapchat'];
    ...
    try {
      const res = await fetch('/api/posts/publish', {
        ...
        body: JSON.stringify({
          ...
          // LinkedIn/Snapchat: pass full image array for multi-image support; connector
          // uses imageUrls when present (falls back to imageUrl for single image).
          ...((platform === 'linkedin' || platform === 'snapchat') && resolvedImageUrls.length > 0 && { imageUrls: resolvedImageUrls }),
        }),
      });
